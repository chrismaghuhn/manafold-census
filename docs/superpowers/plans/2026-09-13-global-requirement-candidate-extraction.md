...
~~~text
PER_TEST_TIMEOUT_ENFORCEMENT = BLOCKED_REQUIRES_TOOLING
~~~
...
* manifest.py — record/trace shard descriptors, identity-set digest,
  analysis manifest, and manifest/index digests with no report references.
...
**Scope:** Add only the thin card-analysis model, source identity key, negative
authority reference, and card-analysis schema. Do not add producers, patterns,
reconciliation, build I/O, CLI, reports, or corpus processing.
...
~~~powershell
python -m pytest tests/test_analysis_model.py -q
~~~
...
### Step 4: Add the normative schema and schema registration

Create schemas/card-analysis.v1.schema.json with strict root properties:
schema, source, outcome, bundle, and no_requirements_basis. Reference the
existing M2 source and bundle definitions through the local validator registry
instead of copying them.

If src/manafold_census/validation.py needs a new registry branch, add only that
branch and a regression test proving existing source-lock and M2 validation
behavior is unchanged.

### Step 5: Run focused checks

~~~powershell
python -m pytest tests/test_analysis_model.py tests/test_resources.py -q
...
~~~powershell
git add src/manafold_census/analysis/__init__.py src/manafold_census/analysis/model.py schemas/card-analysis.v1.schema.json tests/test_analysis_model.py tests/analysis_fixtures.py src/manafold_census/validation.py
git diff --cached --check
git commit -m 'feat: add M3 card analysis record'
~~~
...
10. use NO_REQUIREMENTS_APPLICABLE only after exact negative-authority
...
### Step 4: Implement temporary output and atomic publication
...
worker-count simulation -> byte-equivalent canonical output
...
| Effective pattern definitions and eligibility binding | 3, 5, 6 |
...
| Future byte-equivalent parallelism seam | 5, 10 |
