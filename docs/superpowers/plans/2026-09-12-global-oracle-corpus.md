...
- `src/manafold_census/source/scryfall.py`: observed Scryfall metadata parsing, HTTPS request headers, streamed download, cache validation, and source-lock writing.
...
- `src/manafold_census/cli.py`: minimal `source-discover`, `source-acquire`, `corpus-build`, and offline `corpus-check` commands, preserving `doctor` and `reproduce`.
...
- [ ] Add tests that construct fake discovery and download responses without network access.
...
- [ ] Implement `discover_oracle_cards()` with one HTTPS `urllib.request` request, explicit headers, JSON object/list validation, exactly-one selection, and no fallback dataset.
...
- [ ] Derive the ignored cache filename from the validated upstream source UUID. If an existing file is present, require the expected lock digest/length before reuse and fail on mismatch.
...
- [ ] Resolve `.cache/sources/scryfall/<source_id>.jsonl.gz` from the committed lock without placing a local path in any semantic document.
- [ ] Build the existing `DatasetManifest` (`scryfall-oracle-corpus`, `1.0.0`, `scryfall-oracle-record-index.v1`, actual count), a deterministic `StudySpec`, and an `ArtifactManifest` whose aggregate index digest and total shard byte length represent the 16-file artifact set.
...
- [ ] Add `source-discover`, `source-acquire`, `corpus-build`, and offline `corpus-check` to the existing argparse CLI without a framework dependency; preserve current commands and non-zero error handling.
...
- [ ] Keep `just check` network-independent; add `source-discover`, `source-acquire`, `corpus-build`, and synthetic `corpus-check` recipes.
...
- [ ] Run one live `source-discover`, then one live `source-acquire`; record the observed source ID/update/locator/media type, compressed SHA-256/length, record and shard counts, and aggregate digest without exposing host details.
